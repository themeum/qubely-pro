	...
	$.extend(Plugin.prototype, {
		...
		cloneItems: function () {
			...
			for (let i = 0; i < this._clones; i++) {
				...
				cloneBefore.push(
					this.$element
						.find(`.qubely-carousel-item:nth-child(${this._numberOfItems - i})`)
						.clone(true)
						.addClass("clone")
						.removeClass("active")
				);
				cloneAfter.push(
					this.$element
						.find(`.qubely-carousel-item:nth-child(${i + 1})`)
						.clone(true)
						.addClass("clone")
						.removeClass("active")
				);
		...
		createNavigationController: function () {
			...
			this.nextNavIcon.setAttribute(
				"class",
				this.options.arrowStyle == "arrowright2" ? `fas fa-angle-right` : `fas fa-arrow-right`
			);
			this.nextBtn.append(this.nextNavIcon);

			this.prevNavIcon = document.createElement("span");
			this.prevNavIcon.setAttribute(
				"class",
				this.options.arrowStyle == "arrowright2" ? `fas fa-angle-left` : `fas fa-arrow-left`
			);
		...
		startLoop: function () {
			this.timer = setInterval(() => {
				if (this.isAnimating === false) this.Next();
			...
			if (this.isDragging) {
				...
				for (var i = 0; i < itemCoordinate.length; i++) {
					if (itemCoordinate[i] > newPosition) {
						newPosition =
							this.options.center === true ? itemCoordinate[i] - centerPadding / 2 : itemCoordinate[i];
						found = true;
					}
					if (found === true) break;
				}
		...
		bindEvents: function () {
			...
			if (qubelyCarousel.options.dragable) {
				qubelyCarousel.$outerStage.on(
					"mousedown" + "." + qubelyCarousel._name,
					$.proxy(qubelyCarousel.onDragStart, qubelyCarousel)
				);
				qubelyCarousel.$outerStage.on(
					"mouseup" + "." + qubelyCarousel._name + " touchend" + "." + qubelyCarousel._name,
					$.proxy(qubelyCarousel.onDragEnd, qubelyCarousel)
				);

				qubelyCarousel.$outerStage.on(
					"touchstart" + "." + qubelyCarousel._name,
					$.proxy(qubelyCarousel.onDragStart, qubelyCarousel)
				);
				qubelyCarousel.$outerStage.on(
					"touchcancel" + "." + qubelyCarousel._name,
					$.proxy(qubelyCarousel.onDragEnd, qubelyCarousel)
				);
			}
		...
		onDragStart: function (event) {
			...
			$(document).one(
				"mousemove" + "." + qubelyCarousel._name + " touchmove" + "." + qubelyCarousel._name,
				$.proxy(function (event) {
					$(document).on(
						"mousemove" + "." + qubelyCarousel._name + " touchmove" + "." + qubelyCarousel._name,
						$.proxy(qubelyCarousel.onDragMove, qubelyCarousel)
					);
