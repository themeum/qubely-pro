...
import { IconSave as Icon, ImageSave as Image, ListSave as List, RatingSave as Ratings } from "./components";
...
class Save extends Component {
	...
	renderSections = ({ name, rows }) => {
		const Tag = `t${name}`;
		const classNames = classnames({ "head-row": name === "head" });
		return (
			<Tag>
	...
	renderCellContent = ({
		...
		const classNames = classnames("cell-text");
		...
		switch (type) {
			...
			case "icon":
				return <Icon iconName={iconName} iconCustomColor={iconCustomColor} />;
			case "image":
				return (
					<Image
						image={image}
			...
			case "rating":
				return <Ratings ratings={ratings} ratingsCustomColor={ratingsCustomColor} />;
			default:
				return (
					<RichText.Content
						key={columnIndex}
	...
	render() {
		const {
			attributes: { layout, uniqueId, animation, interaction, fixedWithCells },
		} = this.props;
		const interactionClass = IsInteraction(interaction) ? "qubley-block-interaction" : "";
		const TableContent = this.renderTableContent;
		const classes = classnames("qubely-block-table", layout, interactionClass, { ["fixed-width"]: fixedWithCells });
		return (
