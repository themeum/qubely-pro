...
const Image = (props) => {
	...
	const classNames = classnames("image-cell", "image-wrapper", classes);
	...
	const mediaPreview = !!image.url && (
		<img alt={__("Edit image")} title={__("Edit image")} className={"edit-image-preview"} src={image.url} />
	);
...
const ImageSave = (props) => {
	...
	let img = <img className="qubely-image" src={image.url} alt={"image-type-cell"} style={imageSize} />;

	return <div className={classNames}>{validImage ? img : <div>add image </div>}</div>;
};
