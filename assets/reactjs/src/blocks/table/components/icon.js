...
const { IconList } = wp.qubelyComponents;

const Icon = (props) => {
	const [showIconPicker, toggleIconPicker] = useState(false);
	const { iconName = undefined, classes, onChange, isSelected, isSelectedCell, iconCustomColor } = props;

	const classNames = classnames("icon-cell", "icon-wrapper", classes);
	return (
		<div className={classNames}>
			...
			{isSelected && isSelectedCell && showIconPicker && (
				<Popover position="bottom center" className="qubely-socialicon-picker-popover">
					<IconList disableToggle={true} value={iconName} onChange={(newIcon) => onChange(newIcon)} />
				</Popover>
			)}
		</div>
	);
...
const IconSave = (props) => {
	const { iconName = undefined, classes, iconCustomColor } = props;
