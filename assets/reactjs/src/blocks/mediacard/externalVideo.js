	...
	if (videoUrl) {
		...
		if (videoUrl.match("youtube|youtu.be")) {
			...
			if (videoUrl.match("embed")) {
				...
				id = videoUrl.split(/v\/|v=|youtu\.be\//)[1].split(/[?&]/)[0];
			}
			src =
				...
				"?playlist=" +
				id +
				`&iv_load_policy=3&enablejsapi=1&disablekb=1&autoplay=${autoplay}&mute=1&controls=0&showinfo=0&rel=0&loop=1&wmode=transparent&widgetid=1`;
