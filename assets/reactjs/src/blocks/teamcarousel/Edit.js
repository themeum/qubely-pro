...
class Edit extends Component {
	...
	renderTeams = () => {
		const {
			attributes: { items, layout, carouselItems },
		} = this.props;
		return carouselItems.map((item, index) => {
			return (
				<div key={index} className={`qubely-carousel-item`}>
					{carouselItems.length > items.md &&
						carouselItems.length > items.sm &&
						carouselItems.length > items.sm && (
							<Tooltip text={__("Delete this item")}>
								<span
									className="qubely-repeatable-action-remove"
									role="button"
									onClick={() => this.removeCrouselItem(index)}
								>
					...
					<div className={`qubely-team-carousel-item`}>
						<div className={`qubely-team-${layout}`}>{this.renderAuthorInfo(item, index)}</div>
					</div>
				</div>
			);
		});
	...
	setCarouselLength = (newLength) => {
		const {
			setAttributes,
			attributes: { carouselItems, items },
		} = this.props;
		let newCarouselItems = JSON.parse(JSON.stringify(carouselItems));
		let defaultItem = {
		...
		return (
			<Fragment>
						...
						/> */}
							...
							<PanelBody title={__("Carousel Settings")} initialOpen={false}>
								...
								<Toggle
									label={__("Draggable")}
									value={dragable}
									onChange={(value) => setAttributes({ dragable: value })}
								/>
								...
								{isCentered && (
									<Toggle
										label={__("Fade Deactivated Items")}
										value={activeFade}
										onChange={(value) => setAttributes({ activeFade: value })}
									/>
								)}
							</PanelBody>
							...
							{nav && (
								<PanelBody title={__("Arrow Settings")} initialOpen={false}>
									...
									<Range
										label={__("Shape Size")}
										value={sizeWidth}
										onChange={(value) => setAttributes({ sizeWidth: value })}
										min={1}
										max={100}
										responsive
										unit={["px", "em", "%"]}
									...
									<Tabs>
										<Tab tabTitle={__("Normal")}>
											...
											<ColorAdvanced
												...
												value={arrowShapeColor}
												onChange={(val) => setAttributes({ arrowShapeColor: val })}
											/>
											<Border
												label={__("Border")}
												value={arrowBorderColor}
												onChange={(val) => setAttributes({ arrowBorderColor: val })}
											/>
											<Range
										...
										<Tab tabTitle={__("Hover")}>
											...
											<ColorAdvanced
												label={__("Shape Color")}
												value={arrowShapeHoverColor}
												onChange={(val) => setAttributes({ arrowShapeHoverColor: val })}
											/>
											<Border
												label={__("Border")}
												value={arrowBorderHover}
												onChange={(val) => setAttributes({ arrowBorderHover: val })}
											/>
											<Range
												label={__("Corner Radius")}
												value={cornerHoverRadius}
												onChange={(value) => setAttributes({ cornerHoverRadius: value })}
												min={1}
												max={100}
												responsive
												unit={["px", "em", "%"]}
										...
										{layout !== 2 && (
											<Range
												...
												onChange={(value) =>
													setAttributes(
														layout === 1
															? { avatarVerticalSpacing: value }
															: { avatarHorizontalSpacing: value }
													)
												}
											/>
										)}
							...
							<PanelBody title={__("Name")} initialOpen={false}>
								<Toggle
									label={__("Enable Name")}
									value={enablename}
									onChange={(value) => setAttributes({ enablename: value })}
								/>
								<Color
									label={__("Color")}
									value={nameColor}
							...
							<PanelBody title={__("Designation")} initialOpen={false}>
								<Toggle
									label={__("Enable Designation")}
									value={enableDesignation}
									onChange={(value) => setAttributes({ enableDesignation: value })}
								/>
								<Color
									label={__("Color")}
								...
								<Range
									...
									onChange={(value) => setAttributes({ designationSpacing: value })}
									unit={["px", "em", "%"]}
									max={300}
									min={0}
									responsive
				...
				<div className={`qubely-block-${uniqueId}${className ? ` ${className}` : ""}`}>
					...
					>
						<Carousel options={carouselSettings}>{this.renderTeams()}</Carousel>
