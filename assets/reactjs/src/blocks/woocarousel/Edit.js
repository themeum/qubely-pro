...
const { apiFetch } = wp;
const { useState, useEffect, Fragment, Component } = wp.element;
const { PanelBody, Tooltip, Toolbar, SelectControl, IconButton, Spinner, RangeControl, Placeholder } = wp.components;

const { InspectorControls, RichText, BlockControls, MediaUpload, MediaPlaceholder } = wp.blockEditor;
	...
	const renderImages = (img_url, img_id) => {
		...
		return (
			<div className={className}>
				{img_url ? (
					<img className="qubely-woo_product-image" src={img_url} alt={img_id} />
				) : (
				...
				<InspectorTabs tabs={["style", "advance"]}>
					<InspectorTab key={"style"}>
						...
						<PanelBody
							title={__("Query")}
							initialOpen={false}
							onToggle={() => !categories && getCategoris()}
						>
							<SelectControl
							...
							{categories ? (
								<Dropdown
									...
									onChange={(value) =>
										setAttributes({
											selectedCatagories:
												value.length && value[value.length - 1].label === "All" ? [] : value,
										})
									}
								/>
							) : (
						...
						{nav && (
							<PanelBody title={__("Arrow Settings")} initialOpen={false}>
								<ButtonGroup
									...
									options={[
										[<span className="dashicons dashicons-arrow-right-alt" />, "arrowright"],
										[<span className="dashicons dashicons-arrow-right-alt2" />, "arrowright2"],
									]}
									value={arrowStyle}
								...
								<Range
									label={__("Shape Size")}
									value={shapeWidth}
									onChange={(value) => setAttributes({ shapeWidth: value })}
									min={1}
									max={100}
									responsive
									unit={["px", "em", "%"]}
								...
								<Tabs>
									<Tab tabTitle={__("Normal")}>
										...
										/>
										<Color
											label={__("Navigation Shape Color")}
											value={navShapeColor}
											onChange={(val) => setAttributes({ navShapeColor: val })}
										/>
										<Border
											label={__("Navigation Border")}
											value={navBorderColor}
											onChange={(val) => setAttributes({ navBorderColor: val })}
										/>
										<BorderRadius
											min={0}
											max={100}
									...
									<Tab tabTitle={__("Hover")}>
										...
										<Color
											...
											value={navShapeHoverColor}
											onChange={(val) => setAttributes({ navShapeHoverColor: val })}
										/>
										<Border
											label={__("Border Color")}
											value={navBorderHoverColor}
											onChange={(val) => setAttributes({ navBorderHoverColor: val })}
										/>
										<BorderRadius
											label={__("Corner Radius")}
											value={navHoverRadius}
											onChange={(value) => setAttributes({ navHoverRadius: value })}
											min={1}
											max={100}
											responsive
											unit={["px", "em", "%"]}
								...
								<Tabs>
									...
									<Tab tabTitle={__("Active")}>
										<ColorAdvanced
											...
											onChange={(val) => setAttributes({ dotActiveColor: val })}
										/>
									</Tab>
								</Tabs>
							</PanelBody>
						...
						<PanelBody title={__("Ratings")} initialOpen={false}>
							...
							{showRatings && (
								<Fragment>
									...
									/>
									<Toggle
										label={__("Show Ratings Count")}
										value={showRatingsCount}
										onChange={(val) => setAttributes({ showRatingsCount: val })}
									/>
									{showRatingsCount && (
										<Range
							...
							<Tabs>
								<Tab tabTitle={__("Normal")}>
									...
									<Color
										label={__("Discount")}
										value={discount}
										onChange={(discount) => setAttributes({ discount })}
									/>
								</Tab>
								<Tab tabTitle={__("Hover")}>
									<Color
				...
				<div className="qubely_woo_carousel_wrapper">
					{loading ? (
												...
												<div className="qubely-product-info">
													{showRatings && (
														<div className="qubely-product-ratings-wrapper">
															<div
																className="qubely-product-ratings"
																style={{
																	"--qubely-product-rating": `${
																		average_rating * 20
																	}%`,
																}}
															/>
															{showRatingsCount && rating_count !== 0 && (
																<span className="qubely-ratings-count">
																	{__(`(${rating_count})`)}
																</span>
															)}
														</div>
													)}
													<a className="qubely-product-name" href={permalink}>
														{name}
													</a>
													{on_sale ? (
														<div className="qubely-product-price">
															<div className="qubely-regular-price">
																<s>${regular_price}</s>
															</div>
													...
													<div className="qubely-addtocart-wrapper">
														<RichText
															...
															onChange={(value) =>
																setAttributes({ addToCartButtonText: value })
															}
														/>
													</div>
												</div>
						...
						<div className="qubely-woo_product-not-found">
							<Placeholder icon="admin-post" label={__("Insufficient Products for Carousel")} />
						</div>
					) : (
						<div className="qubely-woo_product-not-found">
							<Placeholder icon="admin-post" label={__("No products found")}>
								<Spinner />
							</Placeholder>
						</div>
					)}
