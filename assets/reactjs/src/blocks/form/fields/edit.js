...
const { PanelBody, TextControl, TextareaControl, Tooltip } = wp.components;

const { ButtonGroup, Range, Select, Separator, Toggle, withCSSGenerator, InspectorTabs, InspectorTab } =
	wp.qubelyComponents;
					...
					return (
						<div className={`qubely-form-field-${type}-option`}>
							...
							<Tooltip text={__("Delete this option")}>
								<span
									className="qubely-action-remove-option"
									role="button"
									onClick={() => updateOptions("delete", index)}
								>
	...
	const handleTimePicker = (type, actionType) => {
		if (type === "hour") {
			if (actionType === "increase") {
				timeFormatType === 12 &&
					hour === 11 &&
					changeseletedTimeFormat(seletedTimeFormat === "AM" ? "PM" : "AM");
				...
				timeFormatType === 12 &&
					...
					changeseletedTimeFormat(seletedTimeFormat === "AM" ? "PM" : "AM");
				setHour(
					(timeFormatType === 12 && hour === 1) || (timeFormatType === 23 && hour === 0)
						? timeFormatType
						: hour - 1
				);
			}
		...
		return (
			<div className="qubely-form-timepicker-wrapper">
				<input
					...
					value={`${hour}:${minute} ${timeFormatType === 12 ? `${seletedTimeFormat}` : ""}`}
					onClick={() => setTimePicker(!showTimePicker)}
					readonly
				/>
		...
		return (
			<Fragment>
				...
				<TextControl
					...
					help={__(
						"You must write field name with hyphen(-) with lowercase. Space, UPPERCASE, Capitalize is not allowed. This name should match with Form template value. Never keep empty this name."
					)}
				/>
							...
							{name === "qubely/formfield-dropdown" && (
								<Fragment>
										...
										return (
											...
											>
												...
												<input
													...
													value={option}
													onChange={(event) =>
														updateOptions("update", index, event.target.value)
													}
												/>
											</div>
										);
									...
									<div className={`qubely-dropdown-add-field-option`}>
										...
										<span
											className={`qubely-action-add-option`}
									...
									<div className="qubely-form-time-picker-format">
										<ButtonGroup
											...
											onChange={(value) => {
												...
												timeFormatType === 23 &&
													changeseletedTimeFormat(hour >= 12 ? "PM" : "AM");
					...
					{type === "textarea" ? (
						...
						<div class="qubely-date-picker-wrapper">
							<input
								...
								autocomplete="off"
								placeholder={placeHolder ? __(placeHolder) : __("dd-M-yy")}
								name={fieldName}
								readonly
							/>
						</div>
