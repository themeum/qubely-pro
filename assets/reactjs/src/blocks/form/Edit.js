...
const { Dropdown, PanelBody, TextControl, Toolbar, Button, Notice, SelectControl, TextareaControl } = wp.components;
...
class Edit extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		const {
			block,
			attributes: { showLabel, labelAlignment, afterSubmitAction },
		} = this.props;
		const { updateBlock, toggleSelection } = dispatch("core/block-editor");

		let changedAttribute =
			showLabel !== prevProps.attributes.showLabel
				? "showLabel"
				: labelAlignment !== prevProps.attributes.labelAlignment
				? "labelAlignment"
				: false;
	...
	submitMailchimp() {
		const {
			attributes: { mcListId, mcMappedFields },
		} = this.props;
	...
	addNewItem(fieldName, newFieldType) {
		...
		innerBlocks.push(
			createBlock("qubely/form-row", { uniqueId: "" }, [
				createBlock("qubely/form-column", { uniqueId: "test2" }, [
					createBlock(`qubely/formfield-${newFieldType}`, {
						fieldName: `${newFieldType}-${innerBlocks.length + 1}1`,
					}),
				]),
			])
		);
			...
			<div className="qubely-form-field-types">
				{!groupField && (
					<div className={"qubely-form-field-tabs"}>
						...
						>
							{icons.from_fields.column} {__("Add Column")}
						</div>
					</div>
				)}
							...
							return (
								...
								>
									{Array(index + 2)
										.fill(0)
										.map(() => {
											return <i style={{ width: `${100 / (index + 1)}%` }} />;
										})}
								</div>
							);
						})}
	...
	renderFormTemplate = () => {
		const {
			clientId,
			attributes: { formItems, afterSubmitAction, mcMappedFields },
		} = this.props;
		...
		return [
			[
				...
				[
					[
						"qubely/form-column",
						{ parentClientId: clientId, fieldSize: "medium" },
						[
							[
			...
			...formItems.map(({ type, label, options, placeHolder, width, required }, index) => {
				return [
					...
					[
						[
							"qubely/form-column",
							{ parentClientId: clientId, fieldSize: "large" },
							[
								[
									`qubely/formfield-${type}`,
									{
										parentClientId: clientId,
										type,
										label,
	...
	renderEmailSettings() {
		const {
			setAttributes,
			attributes: { emailReceiver, emailHeaders, emailFrom, emailSubject, emailBody },
		} = this.props;
		return (
			<PanelBody title={__("Email Settings")} initialOpen={false}>
				<TextControl
					...
					help={__(
						"Enter the recipient email address. This field is mandatory. Without a recipient email, form will not work."
					)}
				/>
				<TextareaControl
	...
	renderMailchimpSettings() {
		const { mc_fields, mc_lists } = this.state;
						...
						<InspectorTab key={"style"}>
							...
							<PanelBody title={__("Input")} initialOpen={false}>
								...
								<Border
									...
									onChange={(val) =>
										setAttributes(
											layout === "classic" ? { inputBorder: val } : { inputBorderMaterial: val }
										)
									}
								/>
							...
							<PanelBody title={__("Settings")} initialOpen={false}>
								...
								<Toggle
									label={__("Enable Policy Checkbox")}
									value={policyCheckbox}
									onChange={(val) => setAttributes({ policyCheckbox: val })}
								/>
								<Toggle
								...
								{reCaptcha &&
									...
									this.state.saved_globally ? (
										...
										<div className="recaptcha-keys">
											...
											<Button
												isPrimary
												onClick={() => this._saveGlobally(reCaptchaSiteKey, reCaptchaSecretKey)}
											>
												{__("Set globally")}
											</Button>
										</div>
							...
							<Dropdown
								...
								renderToggle={({ isOpen, onToggle }) => (
									...
									>
										...
										<span onClick={() => this.setState({ hideDropdown: onToggle })}>
											{" "}
											{__("Add new item")}
										</span>
									</div>
