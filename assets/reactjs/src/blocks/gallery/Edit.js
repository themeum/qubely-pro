...
const { PanelBody, Tooltip, Toolbar, SelectControl, IconButton } = wp.components;
...
class Edit extends Component {
	...
	updateGalleryImage = (key, value, index) => {
		...
		if (key === "add" || key === "delete") {
			let updatedAttributes =
				key === "add"
					? [...galleryContents, { title: __("Best Gutenberg Block") }]
					: galleryContents.slice(0, galleryItems - 1);
			setAttributes({
	...
	renderGalleryItem = () => {
		...
		return [...galleryContents, { image: undefined, title: undefined, addNewItem: true }].map(
			({ title, image, addNewItem = false }, index) => {
				return (
					<div key={index} className={`qubely-gallery-item`}>
						<Tooltip text={__("Delete this item")}>
							<span
								...
								role="button"
								onClick={() => this.moveItem(index, "remove")}
							>
						...
						{index < galleryContents.length && (
							<div className="qubely-gallery-item__move-menu">
								...
								<IconButton
									...
									onClick={() =>
										index === galleryContents.length - 1 ? undefined : this.moveItem(index, "right")
									}
						...
						<div className={`qubely-gallery-content ${enablePopup ? "qubely-gallery-pupup" : ""}`}>
							...
							>
								<div
									className={`qubely-gallery-content-image${
										image != undefined && image.url != undefined ? "" : " qubely-empty-image"
									} qubely-gallery-image-${imageAnimation}`}
								>
									<MediaUpload
										onSelect={(value) => {
											if (addNewItem) {
														...
														<div className="qubely-media-actions qubely-field-button-list">
															...
															<Tooltip text={__("Remove")}>
																<button
																	...
																	onClick={() =>
																		this.updateGalleryImage("image", "", index)
																	}
																	role="button"
																>
																	<span
								...
								{enableCaption == 1 && (
									<div
										className={`qubely-gallery-caption-wrapper ${
											showCaption === "onHover" ? "qubely-gallery-caption-onHover" : ""
										}`}
									>
										<RichText
						...
						<InspectorTab key={"style"}>
							<PanelBody title={__("General Settings")}>
								...
								<Toggle
									...
									value={enablePopup}
									onChange={(value) => setAttributes({ enablePopup: value })}
								/>
								{enablePopup && (
									<Toggle
										label={__("Enable Popup Icon")}
										value={enablePopupIcon}
										onChange={(value) => setAttributes({ enablePopupIcon: value })}
									/>
								)}
								{!enablePopup && (
									<SelectControl
							...
							<PanelBody title={__("Image")} initialOpen={false}>
								{style === 1 && (
									<Fragment>
										<Toggle
											...
											value={enableImgFixedHeight}
											onChange={(value) => setAttributes({ enableImgFixedHeight: value })}
										/>
										{enableImgFixedHeight && (
											<Range
												label={__("")}
												value={imgFixedHeight}
												onChange={(value) => setAttributes({ imgFixedHeight: value })}
												unit={["px", "em", "%"]}
												min={10}
												max={600}
												responsive
								...
								<BorderRadius
									...
									onChange={(val) => setAttributes({ imgBorderRadius: val })}
									min={0}
									max={100}
									unit={["px", "em", "%"]}
									responsive
								...
								<BoxShadow
									label={__("Box-Shadow")}
									value={imgBoxShadow}
									onChange={(val) => setAttributes({ imgBoxShadow: val })}
									disableInset
								/>
								<Select
								...
								{enableCaption == 1 && (
									<Fragment>
										<Padding
											...
											onChange={(val) => setAttributes({ captionPadding: val })}
											min={0}
											max={200}
											unit={["px", "em", "%"]}
											responsive
										...
										<RadioAdvanced
											...
											value={captionVerticalAlign}
											onChange={(value) => setAttributes({ captionVerticalAlign: value })}
											options={[
