...
class Edit extends Component {
	...
	renderAuthorInfo = (item, index) => {
		const {
			attributes: { layout, showAvatar, avatarLayout },
		} = this.props;
	...
	renderTestimonials = () => {
		const {
			isSelected,
			attributes: { layout, showRatings, carouselItems, items, quoteIcon },
		} = this.props;
		...
		return (
			<Fragment>
									...
									<Tabs>
										...
										<Tab tabTitle={__("Hover")}>
											...
											<Border
												...
												value={arrowHoverBorder}
												onChange={(val) => setAttributes({ arrowHoverBorder: val })}
											/>
											<Range
												label={__("Radius")}
							...
							{dots && (
								<PanelBody title={__("Dot Settings")} initialOpen={false}>
									<Range
										label={__("Dot Width")}
										value={dotswidth}
										onChange={(value) => setAttributes({ dotswidth: value })}
										min={1}
										max={100}
										responsive
										unit={["px", "em", "%"]}
										...
										{layout !== 3 && (
											<Styles
												...
												options={[
													{
														value: "left",
														svg: icons.testimonial_avatar.left,
														label: __("Left"),
													},
													{
														value: "right",
														svg: icons.testimonial_avatar.right,
														label: __("Right"),
													},
													{
														value: "top",
														svg: icons.testimonial_avatar.top,
														label: __("Top"),
													},
													{
														value: "bottom",
														svg: icons.testimonial_avatar.bottom,
														label: __("Bottom"),
													},
												]}
											/>
				...
				<div className={`qubely-block-${uniqueId}${className ? ` ${className}` : ""}`}>
					...
					>
						<Carousel options={carouselSettings}>{this.renderTestimonials()}</Carousel>
