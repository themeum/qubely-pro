...
class Edit extends Component {
	...
	renderSlider = (sliderimage, index, addNewItem) => {
		...
		return (
			<div
				className={`qubely-single-img qubely-slider-image-container ${
					sliderimage != undefined && sliderimage.url != undefined ? "" : " qubely-empty-image"
				}`}
			>
				<MediaUpload
					onSelect={(value) => {
									...
									<div className="qubely-media-actions qubely-field-button-list">
										...
										<Tooltip text={__("Remove")}>
											<button
												...
												onClick={() => this.updateAtrributes("sliderimage", "", index)}
												role="button"
											>
	...
	renderImages = () => {
		const {
			attributes: { layout, carouselItems, items, contentVerticalAlign },
		} = this.props;
		return [
			...carouselItems,
			{ sliderimage: null, description: null, slidertitle: null, subtitle: null, addNewItem: true },
		].map((item, index) => {
			return (
		...
		return (
			<Fragment>
							...
							<PanelBody title="" initialOpen={true}>
								...
								{layout !== 6 && layout !== 1 && (
									<Toggle
										...
										value={sliderContent}
										onChange={(value) => setAttributes({ sliderContent: value })}
									/>
								)}
								...
								<PanelBody title={__("Arrow Settings")} initialOpen={false}>
									...
									<Tabs>
										...
										<Tab tabTitle={__("Hover")}>
											...
											/>
											<ColorAdvanced
												label={__("Shape Hover Color")}
												value={arrowShapeHoverColor}
												onChange={(val) => setAttributes({ arrowShapeHoverColor: val })}
											/>
											<Border
												label={__("Border Hover Color")}
												value={arrowBorderHoverColor}
												onChange={(val) => setAttributes({ arrowBorderHoverColor: val })}
											/>
											<Range
										...
										<Fragment>
											...
											<PanelBody title={__("Description")} initialOpen={false}>
												<Toggle
													...
													onChange={(value) => setAttributes({ activeDescription: value })}
												/>
												{activeDescription && (
													<Fragment>
														<Color
				...
				<div className={`qubely-block-${uniqueId}${className ? ` ${className}` : ""}`}>
					...
					>
						<Carousel options={carouselSettings}>{this.renderImages()}</Carousel>
